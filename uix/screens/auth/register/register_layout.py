"""EN: Register screen layout wrapper.
RU: Обертка раскладки экрана регистрации.
"""
...
def apply_register_layout(view) -> None:
    """EN: Apply responsive layout rules to the register view.
    RU: Применить адаптивные правила раскладки к виду регистрации.
    """
    ...
    def _recalc(*_args) -> None:
        ...
        for field_id in (
            ids.register_email_field,
            ids.register_password_field,
            ids.register_password2_field,
        ):
        ...
        apply_bottom_buttons(
            ...
            buttons=[ids.create_btn, ids.to_login_btn],
            btn_h_dp=dp(BTN_H),
            spacing_dp=dp(ZONE_SPACING),
            width_ratio=0.5,
        )
        ids.create_btn.style = "elevated"
        ids.to_login_btn.style = "text"
