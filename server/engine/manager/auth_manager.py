...
from server.app.security.jwt import create_access_token, create_refresh_token, decode_refresh_token
from server.app.security.passwords import hash_password, needs_rehash, verify_password
...
def _normalize_profile_text(value: str | None) -> str:
    """EN: Normalize optional profile text to payload-safe string without placeholder literals.
    RU: Нормализовать необязательный текст профиля в безопасную для payload строку без строк-заглушек.
    """

    return normalize_user_value_for_output(value)
...
def register_user(email: str, psw: str, *, user_agent: str | None = None, ip: str | None = None) -> dict:
    ...
    try:
        with get_session() as session:
            ...
            session.add(ProfileUser(user_id=user.id, login=None, phone=None, telegram=None))
