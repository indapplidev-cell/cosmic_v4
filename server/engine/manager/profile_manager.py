...
from server.engine.domain.profile_math import (
    DEFAULT_CFG,
    calc_balance_delta,
    calc_pay_raw,
    calc_rating,
...
from server.engine.validation.user_value_normalizer import normalize_user_value_for_storage
...
def _ensure_profile_user(session, user_id: int) -> ProfileUser:
    ...
    if obj is None:
        obj = ProfileUser(user_id=user_id, login=None, phone=None, telegram=None)
        session.add(obj)
        session.flush()
    return obj
    ...
    try:
        with get_session() as session:
            ...
            if login is not None:
                normalized_login = normalize_user_value_for_storage(login)
                if normalized_login is not None:
                    obj.login = normalized_login
            if phone is not None:
                normalized_phone = normalize_user_value_for_storage(phone)
                if normalized_phone is not None:
                    obj.phone = normalized_phone
            if telegram is not None:
                normalized_telegram = normalize_user_value_for_storage(telegram)
                if normalized_telegram is not None:
                    obj.telegram = normalized_telegram
...
def clear_profile_user_fields(user_id: int, fields: list[str]) -> dict:
    """EN: Reset selected ProfileUser fields to real absence of value.
    RU: Сбросить выбранные поля ProfileUser к реальному отсутствию значения.
    """
    try:
        user_id_value = int(user_id)
    except Exception:
