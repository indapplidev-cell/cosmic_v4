...
from server.db.sessions.session_factory import engine
...
def _get_alembic_ini_path() -> Path:
    """EN: Resolve absolute path to server db migrations alembic.ini.
    RU: Получить абсолютный путь к server/db/migrations/alembic.ini.
    """

    return Path(__file__).resolve().parents[2] / "db" / "migrations" / "alembic.ini"
