...
from server.engine.manager.ads_manager import get_ads_config, log_ads_event
from server.telegram.services.payout_miniapp_service import (
...
from server.telegram.services.telegram_hub_service import (
    init_telegram_hub,
...
from server.engine.manager.payout_manager import ack_payout_link_code, get_last_payout_link_status, request_payout_link_code
from server.engine.manager.auth_manager import (
    delete_user,
    get_user_snapshot,
    login_user,
    logout_user,
...
from server.engine.manager.profile_manager import (
...
from server.app.api.auth import get_authenticated_user_id, require_same_user
...
from server.engine.manager.rating_manager import get_top_ratings
from server.app.bootstrap.db_schema_guard import get_db_schema_status
...
_PAYBOT_MINIAPP_INDEX = Path(__file__).resolve().parents[2] / "telegram" / "miniapp" / "static" / "index.html"
_PRIVACY_PAGE = Path(__file__).resolve().parents[2] / "telegram" / "miniapp" / "static" / "privacy.html"
_TERMS_PAGE = Path(__file__).resolve().parents[2] / "telegram" / "miniapp" / "static" / "terms.html"
_SUPPORT_PAGE = Path(__file__).resolve().parents[2] / "telegram" / "miniapp" / "static" / "support.html"
