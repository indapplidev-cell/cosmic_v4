"""EN: Pure server-side profile math formulas and anti-cheat checks.
RU: Чистые серверные формулы профиля и проверки античита.
"""
...
class ProfileMathConfig:
    """EN: Configuration constants for profile formulas from DOCX.
    RU: Константы конфигурации формул профиля из DOCX.
    """
...
def clamp01(x: float) -> float:
    ...
    RU: Ограничить число диапазоном [0..1].
    """
...
def norm_log(x: float, x_ref: float) -> float:
    ...
    RU: Лог-нормализация значения по референсу в [0..1].
    """
...
def f3_attempts(attempts: int, k_attempts: float) -> float:
    ...
    RU: Насыщающийся фактор надежности по числу попыток.
    """
...
def triangular_speed_weight(v: float, v_min: float, v_mid: float, v_max: float) -> float:
    """EN: Triangular speed weight with peak at v_mid.
    RU: Треугольный вес скорости с максимумом в v_mid.
    """
...
def cheat_speed(record_pure: int, chis_sec: float, cfg: ProfileMathConfig = DEFAULT_CFG) -> bool:
    """EN: Two-sided anti-cheat check by CHIS speed (fast OR slow).
    RU: Двусторонняя проверка античита по скорости ЧИС (быстро ИЛИ медленно).
    """
...
def cheat_fast_windows(anti_cheat_windows: list[dict], cfg: ProfileMathConfig = DEFAULT_CFG) -> bool:
    """EN: Detect fast-cheat by submitted score/time windows.
    RU: Определить быстрый чит по переданным окнам score/time.
    """

    speed_limit = float(cfg.V_MAX) * (1.0 + float(cfg.EPS_FAST))
    for item in anti_cheat_windows:
        delta_score = int(item.get("delta_score") or 0)
        delta_sec = max(float(item.get("delta_sec") or 0.0), float(cfg.EPS_T))
        if delta_score < int(cfg.CHEAT_MIN_SCORE):
            continue
        if (float(delta_score) / delta_sec) >= speed_limit:
            return True
    return False


def resolve_w_case(*, hit_record: bool, has_reward: bool, cfg: ProfileMathConfig = DEFAULT_CFG) -> float:
    """EN: Resolve 4-case weight from DOCX.
    RU: Выбрать вес одного из 4 случаев по DOCX.
    """
...
def calc_rating(
    *,
    ...
    """EN: Calculate rating (0..1000) and debug factors.
    RU: Рассчитать рейтинг (0..1000) и отладочные факторы.
    """
...
def calc_pay_raw(sis_sec: float, reward_clicks: int, cfg: ProfileMathConfig = DEFAULT_CFG) -> tuple[float, dict]:
    """EN: Calculate raw payout from banners and rewarded clicks.
    RU: Рассчитать чистую выплату по баннерам и rewarded-кликам.
    """
...
def calc_balance_delta(
    *,
    ...
    """EN: Calculate payout-weighted balance delta, always <= pay_raw.
    RU: Рассчитать дельту баланса с весами, всегда <= pay_raw.
    """
