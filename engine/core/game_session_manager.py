...
RU: Хранит только счетчик попыток и возвращает результат поражения на основе
...
class GameSessionManager:
    ...
    RU: Хранит максимум попыток и текущий счетчик оставшихся попыток. Менеджер
    ...
    def __init__(self, max_attempts: int = 3) -> None:
        ...
        RU: Устанавливает максимум попыток и сразу сбрасывает счетчик
        оставшихся попыток к этому значению, чтобы сессия начиналась с полным
        запасом.
        """
    ...
    def reset(self) -> None:
        ...
        attempt count.
        RU: Использует сохраненное значение max_attempts и полностью
    ...
    def register_loss(self) -> LossOutcome:
        ...
        zero or below.
        RU: Уменьшает attempts_left на единицу, пока счетчик не достигнет нуля.
        Возвращает SOFT_RESET, пока попытки еще есть, и GAME_OVER, когда
        счетчик достиг нуля или ниже.
        """
