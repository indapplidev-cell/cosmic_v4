...
class RoadMotionEngine:
    ...
    def step(self, dt, state, size, config):
        ...
        base_speed_y = (config.SPEED * height / 100) * BASE_FPS
        speed_y = base_speed_y * state.speed_y_factor
        spacing_y = config.H_LINES_SPACING * height
        advanced_rows = self._advance_forward(state, dt, speed_y, spacing_y)

        speed_x = (state.current_speed_x * size[0] / 100) * BASE_FPS
        self._apply_lateral(state, dt, speed_x)

        return MotionResult(advanced_rows)

    def _advance_forward(self, state, dt, speed_y, spacing_y):
        """
        ...
        """
        state.current_offset_y += speed_y * dt
        advanced_rows = 0
